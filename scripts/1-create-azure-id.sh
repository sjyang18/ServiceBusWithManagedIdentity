...
export principalid=$(az identity create --name $MANAGED_IDENTITY_NAME --resource-group $MC_RG --query 'principalId' -o tsv)
az role assignment create --role Reader --assignee $principalid --scope /subscriptions/$SUB_ID/resourcegroups/$MC_RG

az identity show -n $MANAGED_IDENTITY_NAME -g ${MC_RG} --query 'clientId' -o tsv